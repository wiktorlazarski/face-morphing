...
class AugmentationPipeline:
    __RESIZE_AUGMENTATIONS = {
        ...
        "pad_0.085": A.CropAndPad(percent=0.085, keep_size=False, p=1),
        "pad_0.07": A.CropAndPad(percent=0.07, keep_size=False, p=1),
        "pad_0.055": A.CropAndPad(percent=0.055, keep_size=False, p=1),
        "pad_0.04": A.CropAndPad(percent=0.04, keep_size=False, p=1),
        "pad_0.02": A.CropAndPad(percent=0.02, keep_size=False, p=1),
        "pad_0.01": A.CropAndPad(percent=0.01, keep_size=False, p=1),
    ...
    __AUGMENTATIONS = {
        ...
        "clahe": A.CLAHE(clip_limit=2.0, tile_grid_size=(4, 4), p=0.05),
        ...
        "channel_shuffle": A.ChannelShuffle(p=0.02),
        "fog": A.RandomFog(
    ...
    def __call__(
        ...
        if self.resize_augmentations is not None:
            resize_augmentation = A.Compose(
                [random.choice(self.resize_augmentations)],
                keypoint_params=A.KeypointParams(format="xy", remove_invisible=False),
            )
            resized_image = resize_augmentation(image=image, keypoints=keypoints)
            augmentation_result = self.augmentations(
                image=resized_image["image"], keypoints=resized_image["keypoints"]
            )
        else:
            augmentation_result = self.augmentations(image=image, keypoints=keypoints)
        return augmentation_result["image"], augmentation_result["keypoints"]
